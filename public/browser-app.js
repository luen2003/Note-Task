    ...
    try {
        ...
        const allTasks = userData
            .map((task) => {
                ...
                const taskNameWithBreaks = name.replace(/\n/g, '<br>'); // Chuyển ngắt dòng thành <br>
                return `<div class="single-task ${completed ? 'task-completed' : ''}">
                            <p><span><i class="far fa-check-circle"></i></span>${taskNameWithBreaks}</p>
...
formDOM.addEventListener('submit', async (e) => {
    ...
    // Chuyển ký tự ngắt dòng thành <br> trước khi gửi
    const taskNameWithBreaks = taskName.replace(/\n/g, '<br>');

    try {
        await axios.post('/api/v1/tasks', { name: taskNameWithBreaks, username: userName });
        showTasks();
        taskInputDOM.value = '';
        formAlertDOM.style.display = 'block';
